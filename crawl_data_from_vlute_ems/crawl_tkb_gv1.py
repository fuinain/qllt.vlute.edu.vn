...
cursor.execute('SELECT magv, hocky FROM crawl')
data = cursor.fetchone()  # Lấy dữ liệu đầu tiên từ kết quả truy vấn

id_giang_vien = str(data[0])
id_hoc_ky = data[1]
...
insert_query = """
INSERT INTO data_crawl_from_vlute_ems (thu, ma_hoc_phan, ten_hoc_phan, giang_vien, phong, tuan_hoc, ngay_hoc, id_giang_vien, id_hoc_ky)
    ...
    for detail in all_data:
        if len(detail) > 1:  # Kiểm tra chi tiết xem có đủ thông tin không
                ...
                if not exists:  # Chỉ insert nếu mã học phần chưa tồn tại
                    insert_query = """INSERT INTO data_crawl_from_vlute_ems (thu, ma_hoc_phan, ten_hoc_phan, giang_vien, phong, tuan_hoc, ngay_hoc, id_giang_vien, id_hoc_ky)
                                      VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                    thu = detail[0]
                    ...
                    cursor.execute(insert_query, (thu, ma_hoc_phan, ten_hoc_phan, giang_vien, phong, tuan_hoc, ngay_hoc, id_giang_vien, id_hoc_ky))
